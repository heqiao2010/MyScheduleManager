...
//		System.out.println(u + "," + v + "," + d1);
	...
	public static void main(String[] args) {
		...
		RSA mRSA = new RSA(P, Q);
		System.out.println(mRSA.toString());
		BigInteger m[] = new BigInteger[100];
		BigInteger c[] = new BigInteger[100];
		BigInteger m1[] = new BigInteger[100];
		for (int i = 0; i < m.length; i++) {
			m[i] = BigInteger.valueOf(1233326320977L + 31 * i);
		}
		...
		for (int i = 0; i < m.length; i++) {
			m1[i] = mRSA.decode(c[i]);
		}

		for (int i = 0; i < m.length; i++) {
			System.out.println(m[i] + ", " + c[i] + ", " + m1[i] + ", "
					+ (0 == m[i].compareTo(m1[i]) ? true : false));
		}
