...
public class AlarmAlertFullScreen extends Activity {
    ...
    String logAlarmInfo( Alarm mAlarm){
    	return "id->" + mAlarm.id + 
    			" label->" + mAlarm.label + 
    			" daysOfWeek->" + mAlarm.daysOfWeek.getCoded() +
    			" hour->" + mAlarm.hour + 
    			" minute->" + mAlarm.minutes + 
    			" alert->" + mAlarm.alert +
    			" vibrate->" + mAlarm.vibrate + 
    			" silent->" + mAlarm.silent +
    			" enabled->" + mAlarm.enabled +
    			" time->" + mAlarm.time;
    }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if( null == mAlarm){
        	mAlarm = new Alarm();
        	Log.e("main", "cannot get Alarm from getParcelableExtra method...");
        } else {
        	Log.i("main", "Alarm:" + logAlarmInfo( mAlarm ));
        }
...
//        setContentView(inflater.inflate(R.layout.alarm_alert, null));
    ...
    private void dismiss(boolean killed) {
        ...
        doAutoDeleteTimeOutAlarm();
        finish();
    }

    private void doAutoDeleteTimeOutAlarm(){
    	// 检查是否该删除响铃后的闹铃
    	if( null!=mAlarm ){
    		if (Alarms.getIsAutoDeleteTimeOutAlarm(AlarmAlertFullScreen.this)) {
    			Alarms.deleteAlarm(this, mAlarm.id);
    		}
    	}
    }
    /**
