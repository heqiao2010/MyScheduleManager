 ...
 * DES算法理论
 ...
 本世纪五十年代以来，密码学研究领域出现了最具代表性的两大成就。其中之一就是1971年美国学者塔奇曼 （Tuchman）和麦耶（Meyer）
 根据信息论创始人香农（Shannon）提出的「多重加密有效性理论」创立的，后於1977年由美国国家标準局颁布的数据加密标準。 
 DES密码实际上是Lucifer密码的进一步发展。它是一种採用传统加密方法的区组密码。它的算法是对称的，既可用於加密又可用於解密。

 美国国家标準局1973年开始研究除国防部外的其它部门的计算机系统的数据加密标準，於1973年5月15日和1974年8月27日先后两次向公
 眾发出了徵求加密算法的公告。 加密算法要达到的目的通常称為DES密码算法要求主要為以下四点：

 提供高质量的数据保护，防止数据未经授权的洩露和未被察觉的修改；具有相当高的复杂性，使得破译的开销超过可能获得的利益，同时又要
 便於理解和掌握 DES密码体制的安全性应该不依赖於算法的保密，其安全性仅以加密密钥的保密為基础实现经济，运行有效，并且适用於多种
 完全不同的应用。

 1977年1月，美国政府颁布：採纳IBM公司设计的方案作為非机密数据的正式数据加密标準（DES枣Data Encryption Standard）。

 目前在这裡，随著三金工程尤其是金卡工程的啟动，DES算法在POS、ATM、磁卡及智能卡（IC卡）、加油站、高速公路收费站等领域被广
 泛应用，以此来实现关键数据的保密，如信用卡持卡人的PIN的加密传输，IC卡与POS间的双向认证、金融交易数据包的MAC校验等，均用
 到DES算法。

 DES算法的入口参数有三个：Key、Data、Mode。其中Key為8个字节共64位，是DES算法的工作密钥；Data也為8个字节64位，是要
 被加密或被解密的数据；Mode為DES的工作方式，有两种：加密或解密。

 DES算法是这样工作的：如Mode為加密，则用Key 去把数据Data进行加密， 生成Data的密码形式（64位）作為DES的输出结果；如
 Mode為解密，则用Key去把密码形式的数据Data解密，还原為Data的明码形式（64位）作為DES的输出结果。在通信网络的两端，双方
 约定一致的Key，在通信的源点用Key对核心数据进行DES加密，然后以密码形式在公共通信网（如电话网）中传输到通信网络的终点，数
 据到达目的地后，用同样的Key对密码数据进行解密，便再现了明码形式的核心数据。这样，便保证了核心数据（如PIN、MAC等）在公共
 通信网中传输的安全性和可靠性。
 ...
 DES算法把64位的明文输入块变為64位的密文输出块，它所使用的密钥也是64位,其功能是把输入的64位数据块按位重新组合，并把输
 出分為L0、R0两部分，每部分各长32位，其置换规则见下表：
 ...
 即将输入的第58位换到第一位，第50位换到第2位，...，依此类推，最后一位是原来的第7位。L0、R0则是换位输出后的两部分，L0是
 输出的左32位，R0 是右32位，例：设置换前的输入值為D1D2D3......D64，则经过初始置换后的结果為：L0=D550...D8；
 R0=D57D49...D7。

 经过26次迭代运算后。得到L16、R16，将此作為输入，进行逆置换，即得到密文输出。逆置换正好是初始置的逆运算，例如，第1位经过
 初始置换后，处於第40位，而通过逆置换，又将第40位换回到第1位，其逆置换规则如下表所示：
 ...
 从子密钥Ki的生成算法描述图中我们可以看到：初始Key值為64位，但DES算法规定，其中第8、16、......64位是奇偶校验位，
 不参与DES运算。故Key 实际可用位数便只有56位。即：经过缩小选择换位表1的变换后，Key 的位数由64 位变成了56位，此
 56位分為C0、D0两部分，各28位，然后分别进行第1次循环左移，得到C1、D1，将C1（28位）、D1（28位）合併得到56位，
 再经过缩小选择换位2，从而便得到了密钥K0（48位）。依此类推，便可得到K1、K2、......、K15，不过需要注意的是，16
 次循环左移对应的左移位数要依据下述规则进行：

 循环左移位数
 1,1,2,2,2,2,2,2,1,2,2,2,2,2,2,1
 以上介绍了DES算法的加密过程。DES算法的解密过程是一样的，区别仅仅在於第一次迭代时用子密钥K15，第二次K14、......，
 最后一次用K0，算法本身并没有任何变化。
 DES算法具有极高安全性，到目前為止，除了用穷举搜索法对DES算法进行攻击外，还没有发现更有效的办法。而56位长的密钥的穷
 举空间為256，这意味著如果一台计算机的速度是每一秒种检测一百万个密钥，则它搜索完全部密钥就需要将近2285年的时间，可见
 ，这是难以实现的，当然，随著科学技术的发展，当出现超高速计算机后，我们可考虑把DES密钥的长度再增长一些，以此来达到更高
 的保密程度。

 由上述DES算法介绍我们可以看到：DES算法中只用到64位密钥中的其中56位，而第8、16、24、......64位8个位并未参与DES
 运算，这一点，向我们提出了一个应用上的要求，即DES的安全性是基於除了8，16，24，......64位外的其餘56位的组合变化
 256才得以保证的。因此，在实际应用中，我们应避开使用第8，16，24，......64位作為有效数据位，而使用其它的56位作為
 有效数据位，才能保证DES算法安全可靠地发挥作用。如果不瞭解这一点，把密钥Key的8，16，24，..... .64位作為有效数据
 使用，将不能保证DES加密数据的安全性，对运用DES来达到保密作用的系统產生数据被破译的危险，这正是DES算法在应用上的误
 区，是各级技术人员、各级领导在使用过程中应绝对避免的，而当今各金融部门及非金融部门，在运用DES工作，掌握DES工作密钥
 Key的领导、主管们，极易忽略，给使用中貌似安全的系统，留下了被人攻击、被人破译的极大隐患。
 DES算法应用误区的验证数据
 ...
 如果把上述的Key换為8个字节的0x31，而Data和Mode均不变，则执行DES 后得到的密文完全一样。类似地，用Key:8个0x32
 和用Key:8个0x33 去加密Data （8 个0x31），二者的图文输出也是相同的：5e c3 ac e9 53 71 3b ba
 我们可以得到出结论：
 Key用0x30与用0x31是一样的；
 Key用0x32与用0x33是一样的，......

 当Key由8个0x32换成8个0x31后，貌似换成了新的Key，但由於0x30和0x31仅仅是在第8，16，24......64有变化，而DES
 算法并不使用Key的第8，16，......64位作為Key的有效数据位，故：加密出的结果是一样的。
 DES解密的验证数据：
 ...
 由以上看出：DES算法加密与解密均工作正确。唯一需要避免的是：在应用中，避开使用Key的第8，16......64位作為有效数
 据位，从而便避开了DES 算法在应用中的误区。
 避开DES算法应用误区的具体操作

 在DES密钥Key的使用、管理及密钥更换的过程中，应绝对避开DES 算法的应用误区，即：绝对不能把Key的第8，16，24...
 ...64位作為有效数据位，来对Key 进行管理。这一点，特别推荐给金融银行界及非金融业界的领导及决策者们，尤其是负责管
 理密钥的人，要对此点予以高度重视。有的银行金融交易网络，利用定期更换DES密钥Key的办法来进一步提高系统的安全性和可
 靠性，如果忽略了上述应用误区，那麼，更换新密钥将是徒劳的，对金融交易网络的安全运行将是十分危险的，所以更换密钥一定
 要保证新Key与旧Key真正的不同，即除了第8，16，24，...64位外其它位数据发生了变化，请务必对此保持高度重视！
...
public class DES {
	...
	public String encrypt(String data) throws UnsupportedEncodingException {
		...
		byte[] bytekey = key.getBytes("UTF-8");// 密钥字节数组
		byte[] bytedata = sbDate.getBytes("UTF-8");// 原始数据字节数组
	...
	public String decrypt(String encryptData) {
		try {
			...
			String deResult = new String(result, "UTF-8");
